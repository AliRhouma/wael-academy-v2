...
| **Matières** `matieres/*` | matière card → `open` · filter tabs (index) → `toggle-on` · head tabs (خطوة بخطوة / الدروس / إمتحانات) → `toggle-on` · chapitre / parcours open · fold → `expand` · `collapse` · type chip (دروس، تمرين…) → `toggle-on` · contenu video & parcours step → `play` · « وثيقة » / « الإصلاح بالفيديو » files → `open` · closing the files sheet → `close` · a file in it → `success` (again → `press`) |
